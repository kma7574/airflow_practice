...
with DAG(
    ...
    def insrt_postgres(postgres_conn_id, tbl_nm, file_nm, **kwargs):
        ...
        postgres_hook.bulk_load(tbl_nm, file_nm)  # 파일을 데이터베이스 테이블에 저장, bl_nm 테이블 존재하지 않으면 오류 뱉음


    insrt_postgres = PythonOperator(
        task_id='insrt_postgres',
        python_callable=insrt_postgres,
        op_kwargs={'postgres_conn_id': 'conn-db-postgres-custom', # postgres connection 정보
